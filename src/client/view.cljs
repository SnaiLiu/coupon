...
(defc of-user-coupons
  ...
  [:div.user-coupons {:key (str group-id "-" (:user/name curr-user-coupons))}
   (for [{:coupon/keys [id name description num] :as coupon} (:user/coupons curr-user-coupons)]
     (let [member-name (:user/name curr-user-coupons)]
       (ui/card {:style {:margin-top "5px"} :key name}
                (ui/card-header {:title name :subtitle description :avatar "img/couponAvator.png"})
...
(defc of-group-info
  ...
  [:div.curr-group
   ...
   (ui/list {:default-value (count (keys members))
             :id            "member-list"}
            (for [member members]
              (ui/list-item {:primaryText (:user/name member)
                             :key         (str name "-" (:user/name member))
                             :style       {:color (if (= username (:user/name member)) "red" "black")}
                             :leftAvatar  (ui/avatar {:src "img/userAvator.png"})
                             :nestedItems [(of-user-coupons dispatch id username member)]})))])
...
(comment
  (def model
    {:user               #:user{:name "柳朕"}
     ...
     :curr-change-coupon nil #_{:group-id 12345 :member-name "姜琳琳" :coupon-id 1234 :coupon-name "洗衣券" :change-num -1}
     :dispatch           #(.alert js/window %)})
